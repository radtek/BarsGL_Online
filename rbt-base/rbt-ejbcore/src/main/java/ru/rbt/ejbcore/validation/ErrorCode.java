...
//    , ACCOUNT_BSA_INVALID(2017, "Не найдена запись в таблице BSAACC для ID (BSAACID) = '%s'")
    ...
    , PLCODE_NOT_CORRECT(2043, "Символ доходов / расходов '%s' не соответствует настройкам для:\nAccountType '%s', Тип собств '%s', Код срока '%s', должен быть '%s'")
    , ACCOUNT2_NOT_CORRECT(2044, "Балансовый счет 2-го порядка '%s' не соответствует настройкам для:\nAccountType '%s', Тип собств '%s', Код срока '%s', должен быть '%s'")
//    GL_SEQ = XX
