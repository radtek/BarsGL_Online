 ...
 * Настройка определения даты проводки
 */
public class PostingDateTest extends AbstractRemoteTest {
    ...
    @Test public void test() throws ParseException {
        ...
        baseEntityRepository.executeNativeUpdate("update gl_oper o set o.fan = ?1, o.STRN = ?2 where o.gloid = ?3", Y.name(), N.name(), operation.getId());
        ...
        baseEntityRepository.executeNativeUpdate("update gl_oper o set o.fan = ?1, o.STRN = ?2 where o.gloid = ?3", N.name(), N.name(), operation.getId());
        ...
        teardownClass();
        baseEntityRepository.executeNativeUpdate("update gl_oper o set o.oper_class = ?1 where o.gloid = ?2", GLOperation.OperClass.MANUAL.name(), operation.getId());
        pod = remoteAccess.invoke(OrdinaryPostingProcessor.class, "calculatePostingDate", operation);
        Assert.assertEquals(DateUtils.parseDate("2012-12-13", "yyyy-MM-dd"), pod);

        baseEntityRepository.executeNativeUpdate("update gl_oper o set o.oper_class = ?1 where o.gloid = ?2", GLOperation.OperClass.AUTOMATIC.name(), operation.getId());
        pod = remoteAccess.invoke(OrdinaryPostingProcessor.class, "calculatePostingDate", operation);
