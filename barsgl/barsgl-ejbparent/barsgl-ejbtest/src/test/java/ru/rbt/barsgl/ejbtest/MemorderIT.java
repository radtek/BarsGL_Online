 ...
 * @fsd 7.10
 */
public class MemorderIT extends AbstractTimerJobIT {

    private static final Logger log = Logger.getLogger(MemorderIT.class);
        ...
        if (Memorder.DocType.MEMORDER == docType) {
            ...
            if (cnt == 0) {
                baseEntityRepository.executeNativeUpdate("insert into F067_MASK (row_id, dat, datto, client_ind, dt_mask, ct_mask)" +
                        " values (1, TO_DATE('2000-01-01','YYYY-MM-DD'), TO_DATE('2029-01-01','RRRR-MM-DD'), 'D', ?, ?)", accDt.substring(0,5) + "%", accCt.substring(0,5) + "%");
            }
        }
