...
public class OfrAccountProcessor extends ValidationAwareHandler<AccountKeys> {
    ...
    public AccountKeys createWrapperAccountKeys(ManualAccountWrapper wrapper) {
        AccountKeys  keys = AccountKeysBuilder.create()
                ...
                .withCurrency("RUR")
                .withCurrencyDigital(currencyRepository.getCurrencyDigital("RUR"))
                .withAccountCode((null != wrapper.getAccountCode()) ? wrapper.getAccountCode().toString() : "")
                .withAccSequence((null != wrapper.getAccountSequence()) ? format("%02d", wrapper.getAccountSequence()) : "")
                .withCustomerType((null != wrapper.getCbCustomerType()) ? wrapper.getCbCustomerType().toString() : "")
                .build();

        // Номер клиента по бранчу
        DataRecord data = operationRepository.getBranchParameters(wrapper.getBranch());
        if (null == data) {
            return keys;
        }
        int cnum = data.getInteger("CNUM");
        keys.setCustomerNumber(format("%08d", cnum));
        keys.setCompanyCode(data.getString("CBCCN"));

        // Счет Майдас
        String acid = wrapper.getAcid();
        if (isEmpty(acid)) {
            try {
                acid = accountRepository.makeMidasAccount(cnum, keys.getCurrency(), keys.getBranch(),
                        wrapper.getAccountCode(), wrapper.getAccountSequence());
            } catch (Exception e) {
                return keys;
            }
        }
        ...
        Date dateOpen = getDateOpen(wrapper);
        data = accountRepository.getAcc2ByAcid(acid, dateOpen);
        if (null == data) {
            return keys;
        }
        String acc2 = isEmpty(wrapper.getBalanceAccount2()) ? data.getString("ACC2") : wrapper.getBalanceAccount2();
        keys.setAccount2(acc2);
        keys.setPassiveActive(data.getString("PSAV"));

        String acod = wrapper.getAccountCode().toString();
        short sq = wrapper.getAccountSequence();
        short custType = 0;                     // сначала ищем независимо от типа собственности
        String plCode = wrapper.getPlCode();
        if (isEmpty(plCode)) {
            plCode = accountRepository.getPlCode(acod, sq, custType, dateOpen);
            if (StringUtils.isEmpty(plCode)) {      // не нашли, ищем для заданного типа собственности
                custType = wrapper.getCbCustomerType();
                plCode = accountRepository.getPlCode(acod, sq, custType, dateOpen);
            }
            if (!isEmpty(plCode)) {                 // установить тип собственности по которому найден PLCODE
                keys.setCustomerType(Short.toString(custType));
            }
        }
