...
public class OperEventHistoryForm  extends OperSuperBase {
    ...
    private void reconfigure() {
        ...
        abw.addAction(quickFilterAction = new DateIntervalQuickFilterAction(grid, colProcDate));
    ...
    protected Table prepareTable() {
        ...
        result.addColumn(col = new Column("OTS", Column.Type.DATETIME, "Дата создания", 130));
        ...
        result.addColumn(col = new Column("OTS_AU2", Column.Type.DATETIME, "Дата подписи", 130));
        col.setFormat("dd.MM.yyyy HH:mm:ss");
        result.addColumn(new Column("USER_AU3", Column.Type.STRING, "Логин 3 руки", 100));
        result.addColumn(col = new Column("OTS_AU3", Column.Type.DATETIME, "Дата подтверж.", 130));
        col.setFormat("dd.MM.yyyy HH:mm:ss");
        result.addColumn(new Column("USER_CHNG", Column.Type.STRING, "Логин изменения", 100));
        result.addColumn(col = new Column("OTS_CHNG", Column.Type.DATETIME, "Дата изменения", 130));
        col.setFormat("dd.MM.yyyy HH:mm:ss");
        ...
        result.addColumn(new Column("OTS_SRV", Column.Type.DATETIME, "Ответ от АБС", 72));
    ...
    protected String prepareSql() {
        String whereSFpart = "";
        String where = SecurityChecker.checkAction(SecurityActionCode.OperHistory)
                ? ""
                //: getSourceAndFilialPart("where", "PST.SRC_PST", "PST.CBCC_CR", "PST.CBCC_DR") + " and PST.INVISIBLE <> '" + InvisibleType.H.name() + "' ";
                : (isEmpty(whereSFpart = getSourceAndFilialPart("where", "PST.SRC_PST", "PST.CBCC_CR", "PST.CBCC_DR")) ? " where" : whereSFpart + " and") +
        ...
        return
                ...
                "PST.INP_METHOD, PST.ID_PAR, PST.ID_PREV, PST.SRV_REF, PST.OTS_SRV, PST.SRC_PST, " +
